...
for ii=1:8
    rdata_struct{ii}=importdata(sprintf('p1_sf10_%d_IQ.csv', ii));
...
data=rdata_struct{5};
    ...
    temp=simple_bandpass(temp, BP, r);
    maxi=0;
    maxval=0;
...
data=rdata_struct{1};
    ...
    upspecial=simple_bandpass(up,BP,r);
    downspecial=simple_bandpass(down,BP,r);
...
for rec=2:4
    ...
    for i=maxloc-2000:maxloc+2000
        if(i++length(appSpecial)-1<length(temp))
...
data=rdata_struct{5};
    ...
    [maxval maxloc]=max(maxi(round((ps_struct(1)+rval2-rval1)/1000-packet_size/8000):round((ps_struct(1)+rval2-rval1)/1000+packet_size/8000)));
    maxi=zeros(4001,1);
    maxloc=maxloc-1+round((ps_struct(1)+rval2-rval1)/1000-packet_size/8000);
    maxloc=maxloc*1000+1;
    for i=maxloc-2000:maxloc+2000
        if(i+length(appSpecial)-1<length(temp))
        maxi(i-maxloc+2001)=max(abs(fftshift(fft(temp(i:i+length(appSpecial)-1).*appSpecial))));
